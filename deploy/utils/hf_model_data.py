...
class BucketPopulatorResource(Construct):
    def __init__(self, scope: Construct, id: str,  bucket_name, hf_model_id):
        ...
        invoker = cr.AwsCustomResource(self, 
                                       ...
                                       on_create = self.create(self.lambda_fn.function_name, bucket_name, hf_model_id),
    ...
    def create(self, lambda_name, bucket_name, hf_model_id):
        lambda_params = {}
        lambda_params['FunctionName'] = lambda_name
        lambda_params['InvocationType'] = "RequestResponse"
        payload = "".join( ["{\"hf_model_id\":\"", hf_model_id, "\", \"bucket_name\":\"",bucket_name, "\"}"])
...
class PopulatedBucketResource(Construct):
    ...
    def __init__(self, scope: Construct, id: str, bucket_name, hf_model_id):
        ...
        self.core_model_name=Fn.select(1, Fn.split(source=hf_model_id, delimiter="/"))
        ...
        bucket_populator= BucketPopulatorResource(self, "InvokerResource",  bucket_name, hf_model_id)
        ...
        self.model_archive_key="".join(["embedding-models/",self.core_model_name,"/model.tar.gz"])
        CfnOutput(scope=self, id=f"BucketName", value=f"{self.default_bucket.bucket_name}")
        CfnOutput(scope=self, id=f"BucketArchive", value=f"{self.model_archive_key}")
